...
class AsyncEngine:
    ...
    async def wakeup(self, tags: list[str] | None = None):
        ...
        if any(tag not in self.sleeping_tags for tag in tags):
            ...
            return
        if self.backend == 'pytorch' and hasattr(self.engine, 'wakeup_resources'):
            fully_awake = await asyncio.to_thread(self.engine.wakeup_resources, tags)
            if fully_awake:
                self.engine.resume_after_wakeup()
        else:
