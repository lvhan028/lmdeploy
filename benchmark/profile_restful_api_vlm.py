...
def prepare_images():
    ...
    if args.resolution == 'random':
        ...
        random_array = np.random.randint(0, 256, (args.height, args.width, 3), dtype=np.uint8)
        image = Image.fromarray(random_array, 'RGB')
        _images.append(image)
...
def sample_sharegpt_requests(
    ...
    filtered_dataset: List[Tuple[str, int, int, int, int]] = []
    for i in range(len(dataset)):
        ...
        # if args.resolution == 'random':
        #     index = np.random.randint(0, len(resolutions))
        #     height, width = resolutions[index]
        # else:
        #     height, width = args.height, args.width
        # filtered_dataset.append((prompt, prompt_len, output_len, height, width))
...
def sample_random_requests(
    ...
    if True:
        ...
        for i in range(num_prompts):
            ...
            # else:
            #     height, width = args.height, args.width
            # input_requests.append((prompt, int(input_lens[i]), int(output_lens[i]), height, width))
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resolution',
                        ...
                        default='random',
                        help='Set image resolution mode')
    parser.add_argument('--width',
                        type=int,
                        default=448,
                        help='the width of an image, which is used when --resolution is stable')
    parser.add_argument('--height',
                        type=int,
                        default=448,
                        help='the height of an image, which is used when --resolution is stable')
