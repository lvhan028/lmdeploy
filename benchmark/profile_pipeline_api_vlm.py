...
def get_image(height, width):
    image = image_pool.get((height, width), None)
    if image:
        return image
    random_array = np.random.randint(0, 256, (height, width, 3), dtype=np.uint8)
    image = Image.fromarray(random_array, 'RGB')
    image_pool[(height, width)] = image
    return image
...
class Engine:
    ...
    def process_request(self, requests, resolutions, profiler: Profiler, temperature, top_p, top_k, stream_output):

        prompts = [('describe the image', get_image(*resolution))
                   for (prompt, _, _), resolution in zip(requests, resolutions)]
        gen_configs = [
...
def sample_resolution(mode, w, h, n):
    if mode == 'stable':
        # w = os.environ.get('IMAGE_WIDTH', 448)
        # h = os.environ.get('IMAGE_HEIGHT', 448)
        # w = int(w)
        # h = int(h)
        return [(h, w)] * n
    elif mode == 'random':
        ...
        indices = [random.randint(0, len(candidates) - 1) for _ in range(n)]
        return [candidates[i] for i in indices]
    else:
        raise ValueError(f'unsupported resolution choice "{mode}"')
...
def main():
    ...
    resolution = sample_resolution(args.resolution, args.width, args.height, args.num_prompts)
